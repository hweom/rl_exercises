...
impl Tiling {
    ...
    // Returns a vector with size equal to feature_count() where only one
    // element is 1.0 and all the others are 0.0.
    fn get_feature(&self, pc: &DVector<f64>, pi: &DVector<i32>) -> usize {
        ...
        for i in (0..pc.len()).rev() {
            ...
            println!("x: {}, origin: {}, step: {}, index: {}", x, p.origin, step, index);
            offset = offset * p.step_count + index;
        }
...
impl TilingSet {
    ...
    pub fn from_dimensions(
        ...
        let mut origin = DVector::from_iterator(c_len, continuous_dimensions.iter().map(|d| d.bounds.min));
        ...
        let offset_step = DVector::from_iterator(
            c_len,
            (0..c_len).map(|i| step_size[i] / count as f64),
        );
        for _ in 0..count {
            let tiling = Tiling {
                continuous_partitions: (0..c_len)
                    ...
                    .collect(),
                integer_partitions: (0..i_len)
                    .map(|i| IntegerPartition {
                        origin: integer_dimensions[i].min,
                        step_count: (integer_dimensions[i].max - integer_dimensions[i].min)
                            as usize,
                    })
                    .collect(),
            };
    ...
    pub fn feature_count(&self) -> usize {
        self.tilings.iter().map(|t| t.feature_count()).sum()
    }

    // Returns the active features for a given point in the state space.
    // Dimension of the return vector is equal to the number of tilings, or count().
    pub fn get_features(&self, pc: &DVector<f64>, pi: &DVector<i32>) -> Vec<usize> {
        let mut feature_indices = Vec::with_capacity(self.tilings.len());
        let mut index_offset = 0;
        for t in &self.tilings {
            feature_indices.push(t.get_feature(pc, pi) + index_offset);
            index_offset += t.feature_count();
        }
        feature_indices
    }
}
...
mod tests {
    ...
    fn single_tiling_1d() {
        ...
        assert_eq!(tilings.feature_count(), 10);

        assert_eq!(tilings.get_features(&pc(&[0.0]), &pi(&[])), vec![0]);
        assert_eq!(tilings.get_features(&pc(&[-1.0]), &pi(&[])), vec![0]);
        assert_eq!(tilings.get_features(&pc(&[9.5]), &pi(&[])), vec![9]);
        assert_eq!(tilings.get_features(&pc(&[11.0]), &pi(&[])), vec![9]);
    }
    ...
    fn single_tiling_2d() {
        ...
        assert_eq!(tilings.count(), 1);
        assert_eq!(tilings.feature_count(), 200);

        assert_eq!(tilings.get_features(&pc(&[-1.0, 0.0]), &pi(&[])), vec![9]);
        assert_eq!(tilings.get_features(&pc(&[-1.0, 1.0]), &pi(&[])), vec![29]);
    }
    ...
    fn single_tiling_1i() {
        ...
        assert_eq!(tilings.count(), 1);
        assert_eq!(tilings.feature_count(), 5);

        assert_eq!(tilings.get_features(&pc(&[]), &pi(&[0])), vec![0]);
        assert_eq!(tilings.get_features(&pc(&[]), &pi(&[-1])), vec![0]);
        assert_eq!(tilings.get_features(&pc(&[]), &pi(&[1])), vec![1]);
        assert_eq!(tilings.get_features(&pc(&[]), &pi(&[4])), vec![4]);
        assert_eq!(tilings.get_features(&pc(&[]), &pi(&[5])), vec![4]);
    }
    ...
    fn single_tiling_1d_1i() {
        ...
        assert_eq!(tilings.count(), 1);
        assert_eq!(tilings.feature_count(), 50);

        assert_eq!(tilings.get_features(&pc(&[0.0]), &pi(&[0])), vec![0]);
        assert_eq!(tilings.get_features(&pc(&[0.0]), &pi(&[1])), vec![10]);
    }
    ...
    fn multi_tiling_1d_1i() {
        ...
        assert_eq!(tilings.feature_count(), 150);

        // Point (0, 0) should be in tile 0 on all tilings.
        assert_eq!(tilings.get_features(&pc(&[0.0]), &pi(&[0])), vec![0, 50, 100]);

        // Offset step is 1.0/3, so point 1.4 should be on tile 1 for tilings
        // 0 and 1, but tile 0 on tiling 2.
        assert_eq!(tilings.get_features(&pc(&[1.4]), &pi(&[0])), vec![1, 51, 100]);
